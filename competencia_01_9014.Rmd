...
9014 (para test de 9005)

HT (9002)
dataset: fe1 (lags delta 1,2,3)
11 hiperparámetros
train 1,2,3
0.2
auc 0.9441
...
#levanto dataset con lags 1,2,3 
...
# # Identificar columnas a eliminar
# columnas_eliminar <- grep("lag4|lag5|delta4|delta5", names(dataset), value = TRUE)
# 
# # Visualizar las columnas que se van a eliminar
# print(paste("Total de columnas a eliminar:", length(columnas_eliminar)))
# print(columnas_eliminar)
...
```{r}
# # EXP HT 9006 (en colab) AUC = 0.9472
# 
...
#   num_leaves = 455,
#   min_data_in_leaf = 3 #alterado. daba 4
# )
```
...
```{r}
# 9010 dataset fe1  bayesiana AUC = 0.9457

# PARAM$out$lgbm$mejores_hiperparametros <- list(
#   num_iterations = 4670,
#   learning_rate = 0.01715264,
#   feature_fraction = 0.8654658,
#   num_leaves = 393,
#   min_data_in_leaf = 3,
#   max_depth = 15,
#   min_gain_to_split = 0.002911684,
#   lambda_l1 = 0.2110022,
#   lambda_l2 = 3.550555,
#   bagging_fraction = 0.8114441,
#   bagging_freq = 7
# )
```
...
# para VM
ggsave(paste0("feature_importance_ggplot_exp_", PARAM$experimento, ".png"),
       plot = p,
       width = 12,
       height = 8,
       dpi = 300)
...
# # para VM
# ggsave(paste0("curva_ganancia_exp_", PARAM$experimento, ".png"), 
#        plot = p, 
#        width = 10, 
#        height = 6, 
#        dpi = 300)
...
# Identificar el punto de máxima ganancia
max_ganancia <- max(resultados_ganancia$ganancia_total)
punto_optimo <- resultados_ganancia[ganancia_total == max_ganancia]

# Definir el límite inferior del eje Y como 50% de la ganancia máxima
limite_y_min <- max_ganancia * 0.5

pbest <- ggplot(resultados_ganancia, aes(x = envios, y = ganancia_total)) +
  geom_line(color = "#2C7BB6", size = 1) +
  geom_point(color = "#2C7BB6", size = 2) +
  # Marcar el punto óptimo
  geom_point(data = punto_optimo, aes(x = envios, y = ganancia_total),
             color = "#D7191C", size = 4, shape = 21, fill = "#D7191C") +
  # Agregar etiqueta al punto óptimo
  geom_text(data = punto_optimo, 
            aes(x = envios, y = ganancia_total,
                label = paste0("Óptimo: ", scales::comma(envios), " envíos\n$", 
                              scales::comma(round(ganancia_total)))),
            vjust = -1.5, hjust = 0.5, size = 3.5, fontface = "bold", color = "#D7191C") +
  geom_hline(yintercept = 0, linetype = "solid", color = "gray50", alpha = 0.3) +
  scale_x_continuous(labels = scales::comma) +
  scale_y_continuous(labels = scales::comma, limits = c(limite_y_min, NA)) +
  labs(
    title = paste0("Curva de Ganancia best - EXP ", PARAM$experimento),
...
# para Rstudio
ruta_curva <- file.path(dir_experimento,
                        paste0("curva_ganancia_best_exp_", PARAM$experimento, ".png"))
ggsave(ruta_curva, p, width = 10, height = 6, dpi = 300)

# # para VM
# ggsave(paste0("curva_ganancia_best_exp_", PARAM$experimento, ".png"), 
#        plot = p, 
#        width = 10, 
#        height = 6, 
#        dpi = 300)
