...
HT 
dataset: fe123rank agui (rank todas continuas + agui backpropagado)
...
PARAM$hyperparametertuning$hs <- makeParamSet(
  makeIntegerParam("num_iterations", lower= 800L, upper= 4300L), #acorté
  # o más con early stopping al entrenar
  makeNumericParam("learning_rate", lower= 0.006, upper= 0.15), #agrandé
...
```{r chunk74}
#EXP 9016 hasta iter 16 auc 0.9417
...
dir_experimento <- "C:/Users/Silvana/Documents/Maestria Exactas/DMEyF/competencia 1/experimentos/exp9015"
...
cat("\n=== Probando predicción con cada modelo ===\n")

dfuture_matrix <- data.matrix(dfuture[, campos_buenos, with = FALSE])

cat("Dimensiones de dfuture_matrix:", nrow(dfuture_matrix), "filas x", 
    ncol(dfuture_matrix), "columnas\n\n")

for(i in 1:length(PARAM$semillas)) {
  cat("Probando modelo", i, "(semilla", PARAM$semillas[i], ")... ")
  
  resultado <- tryCatch({
    pred_test <- predict(
      modelos_ensemble[[i]],
      dfuture_matrix[1:10, ]  # Solo primeras 10 filas para probar
    )
    cat("OK - Predicción exitosa\n")
    "OK"
  }, error = function(e) {
    cat("ERROR:", e$message, "\n")
    e$message
